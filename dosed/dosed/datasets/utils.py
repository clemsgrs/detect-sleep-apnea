...
def get_train_validation(h5_directory,
                         percent_validation,
                         seed=None):
    ...
    random.seed(seed)
    index_validation = int(len(records) * percent_validation / 100)
    random.shuffle(records)
    validation = records[:index_validation]
    train = records[index_validation:]
