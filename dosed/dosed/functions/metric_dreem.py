...
def compute_f1_score(event_pred, event_true):
    ...
    total_fn = 0
    if len(event_pred) == 0:
        total_tp, total_fp, total_fn = 0, 0, len(event_true)
    if len(event_true) == 0:
        total_tp, total_fp, total_fn = 0, len(event_pred), 0
    else:
        tp, fp, fn = compute_tp_fp_fn_for_each_entry(event_pred, event_true)
        total_tp += tp
        total_fp += fp
        total_fn +=fn
